...
signal waveforms from all sensing wires are amplified and digitized by the front-end electronics, and transmitted to the data acquisition system (Chapter~\ref{ch:trig}) outside of the cryostat~\cite{cryo_10719}.

The front-end mother boards and digital multiplexer boards from the Cold Electronics subsystem (Chapter~\ref{ch:ce}) are directly mounted on the APAs as part of the APA assembly (shown in Figure~\ref{fig:elec_CMBonAPA}).  The Photon Detectors (Chapter~\ref{ch:photon}) are also mounted 
inside the APAs' frame openings before the APAs are installed into the cryostat. The TPC subsystem also interfaces to the cryostat and cryogenics system~\cite{cryo_10719} through the TPC mounting fixtures, and to the DAQ 
...
\begin{cdrfigure}[Cross section of the TPC inside the cryostat]{tpc-xsect1}{Cross section of the 5-kt fiducial mass LBNE TPC inside the cryostat.  The length of the TPC is  30~m along the direction of the neutrino beam (into the paper)}
...
Its three rows of CPA planes interleaved with two rows of APA planes~\footnote{Note that the APA and 
CPA positions are exchanged in the DUNE design.} 
...
edge-to-edge 
along the beam direction, forming the 30-m active length of the detector.  Each CPA has the same width, but half the height ($\sim$3~m) as an APA, for ease of assembly and transportation.  Four CPAs stack vertically to form the full 12-m active height. 
...
The requirements for the TPC can be found in the requirements documentation \cite{lar-fd-req}. The most significant ones are the following. The TPC shall:
...
\item Provide redundancy in the discrimination of electrons from photon conversions and ensure long-term reliability over the life of the experiment;  the configuration will use three instrumented wire planes
...
\end{itemize} %%%%%%%%%%%%%% Anne 4/22

Both stainless-steel and copper-beryllium (CuBe) wires are potential candidates.  
Stainless steel was the choice of ICARUS~\cite{Icarus-T600}, while a copper-plated 
stainless-steel wire was chosen by MicroBooNE~\cite{microboonecdr} (to reduce resistance).  Both experiments use a wire-termination 
...
If the APA were cooled abruptly from room temperature to LAr 
temperature the wire tension could go up from 5~N to as much as 12~N due 
to the lag between thermal contraction of the wires and that of the 
frame.  Since the yield is around 30~N this would not threaten the 
wires.  The frame is also rigid enough that it would not be harmed.  
However, any abrupt changes of load are reason for concern so the APAs 
should be cooled gradually during cooling cycles.
