...
{Individual photon collection efficiencies}
 Factor & Description & Value & Comments \\ \toprowrule
   $\epsilon_{geom}$ & geometric acceptance & 0.0047 & FD optical simulation  \\ \colhline
      $\epsilon_{E}$ & field correction & 0.6 & 500~V/cm  \\ \colhline
      $\epsilon_{mesh}$ & TPC wire shadowing & .70
      & average value  \\ \colhline
      $\epsilon_{conv}$ & TPB conversion & 1 & see
      ref.~\cite{bib:gehman}  \\ \colhline
      $\epsilon_{capt}$ & & 0.5 & about half
      converted photons\\ 
      &  & & travel into waveguide  \\ \colhline
      $\epsilon_{tran}$ & waveguide transmission & 0.0005 - 0.0078 & prototype
      dependent  \\ \colhline
     $\epsilon_{QE}$ & SiPM QE & .41  & SensL C-series  \\
\end{cdrtable}

Based on simulations of photon transport and the efficiency factors
listed in Table 5.1, and using only the prompt light, the best
prototypes yet designed are capable of seeing proton decay with good
efficiency (~95\% across the full volume) and supernova events down to
10 MeV with some efficiency (~30\% for a 5 MeV~electron).  Of course
this value is very position-dependent, as the geometric acceptance,
wire shadowing and transport corrections all depend on the location of
the event. It is also expected that there will be a thousand or so
events in 10 kt during the SN burst neutrino arrival and hence
detection efficiency may be higher due to the increase in number of
photons - although for triggering the leading edge of neutrino
arrivals is most important.

An additional possibility is to utilize the delayed photons to
...
understand the contribution of backgrounds coming from SiPM dark rate
and $^{39}$Ar during the integration window. When late light is
included, the detection efficiency for proton decay is >99\% even at
the furthest point from the photon detectors and the efficiency for
5~MeV electrons rises to about 70\%.

Figure~\ref{fig:photon_map} shows the probability of energy deposition
from MIPs being detected in the photon detectors.

\begin{cdrfigure}[Probability of photon being detected in
  detectors when depositing energy]{photon_map}{Photon map giving the
    number of photons expected per MeV deposited energy accounting for
    the efficiencies listed in table~\ref{example}} 
  \includegraphics[width=0.4\textwidth]{projX51_phperMeV_early.png}
  \includegraphics[width=0.4\textwidth]{projZ35_phperMeV_early.png}
\end{cdrfigure}
...
\begin{cdrfigure}[Schematic drawing of a light guide with its
    ...
    \includegraphics[width=.85\columnwidth]{WaveguideSketch}
\end{cdrfigure}

The wavelength shifter converts incident VUV scintillation photons
that strike it into 430~nm photons inside the bar, with an efficiency of $\sim$50\% %of
...
\paragraph{Alpha source runs:} The alpha source was placed at a
...
To mitigate the reduced attenuation length coming from acrylic and
...
A testing program is underway at Fermilab, CSU, and IU to provide data
...
Following initial tests, SenSL released a new model of the device we
...
In the last 18 months, a test setup for SiPM evaluation has been
...
SenSL C series SiPMs were also tested for cross-talk. Cross-talk is
...
The reference design for mounting the PDs into the APA frames calls
...
Three different methods of making these electrical contacts have been studied:
\begin{itemize}
\item Soldering
...
In the 35t prototype, each SiPM signal is transmitted on an
...
In the reference design, the front-end electronics reside outside of
...
In tests to date, the SSP has demonstrated the ability to measure
...
Several ASICs have been designed over the last few years for SiPM
...
In the LBNE prototypes (i.e., the 35t) and in future far detector it
